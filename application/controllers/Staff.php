...
class Staff extends CI_Controller
{
	...
	public function acdprofile($user_id)
	{
		$this->load->model('staff_model');
	...
	public function personalprofile($user_adno)
	{
		$this->load->model('student_model');
	...
	public function semester_request_list()
	{
		$staff_id = $this->session->userdata('user_id');
		$this->load->model('staff_model');
		$posts = $this->staff_model->semester_request_list($staff_id);
		$this->load->view('staff/semester_request_listing', array('posts' => $posts));
	}

	public function sem_action($id)
	{
		$this->db->query("update sem_reg set status='approved' where sem_reg_id=$id");
		redirect(base_url('Staff/semester_request_list'));
	}
